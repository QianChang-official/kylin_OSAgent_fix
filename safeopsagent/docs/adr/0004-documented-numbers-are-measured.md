...
**三、同一个错误又犯了一次。**
修正教训二时，把对抗基准的"执行数"与"跳过数"也写进了断言表。本机（Windows）
全绿，推上去 CI（Linux）全红：Linux 上 64 条全部执行、0 跳过，Windows 上 63 执行、
1 跳过。这两个量和 `N passed` 是同一类东西——**环境相关**，而我刚在教训一里写完
不能用它们做基准，转头就用了。

由此得出一条可机械执行的判据，写在 `CLAIM_PATTERNS` 上方：

> 换一台机器重跑，这个数会不会变？会变就不能当门禁基准，也不该当文档里的
> 门面数字。

基准的**定义数**（`benchmark_total`）与用例的**收集数**（`test_collected`）是项目
属性；执行数、跳过数、通过数是某次运行在某台机器上的结果。文档只写前者，后者
需要时以"随平台变化"的措辞给出，不给具体数字。
